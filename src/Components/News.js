...
export class News extends Component {
  // Correct static defaultProps and propTypes
  static defaultProps = {
    country: 'in',
    pageSize: 8,
    category: 'general'
  ...
  fetchMoreData = async () => {
    const nextPage = this.state.page + 1; // Increment page first
    this.setState({ loading: true }); // Show spinner while fetching
    ...
    try {
      ...
      this.setState({
        page: nextPage, // Update page state
        loading: false,
            ...
            <div className="container">
              <div className="row">
                {this.state.articles.map((element) => (
                  element.source.name !== "[Removed]" && <div className="col-md-3" key={element.url}>
                    <NewsItem
                      title={element.title}
